...
| 경로 | 화면 | 내용 |
|---|---|---|
| `/` | 대시보드 | 부문·기간 필터, KPI 4종, 만료·갱신 타임라인, 조항 노출 현황, 최근 처리 계약 |
| `/analyze` | 계약서 분석·요약 | 샘플 계약서 첨부 → 4단계 분석 → 좌측 원문(PDF·OCR)과 대조하며 분류·플레이북 차이·3층 요약·대장 필드 확인 |
...
기획서의 `F-LDG-005 대시보드·집계` 를 따라 구성했고, 블록마다 As-Is 손실 하나에 대응합니다.

| 블록 | 답하는 문제 |
|---|---|
| 부문·기간 필터 | 현업 담당자는 소속 부문 집계만 조회 — 건수·금액으로 정보가 새지 않게 (`F-SEC-002`) |
| KPI 4종 | 데이터화 완료·대장 필드 확정률·검토 대기·계약금액 총계 |
| 만료·갱신 타임라인 | `P2` 만료·자동갱신 누락. 눈금은 알림 시점 D-120/90/60/30/7 (`F-LDG-004`) |
| 조항 노출 현황 | `P1` 조항 검색 불가 · `P5` 리스크 집계 불가 — 불리 조항 **보유**와 필수 조항 **부재**를 같이 셉니다 |
| 최근 처리 계약 | 대장에 막 반영된 문서 — 행을 누르면 계약 상세로 |

같은 스펙의 예외 규칙("미확정 데이터가 포함된 집계는 그 비율을 함께 표시")을 지키려고 모든 집계에 `확정 N · 미확정 N` 을 병기합니다. 부문별 수치의 합은 전사 수치와 맞춰두었습니다.

## 시연 시나리오 (3~5분)

1. **대시보드** 에서 계약이 필드가 되었을 때 답할 수 있게 되는 것들을 보여줍니다 — 표준을 벗어난 조항이 몇 건인지, 있어야 할 조항이 빠진 계약이 몇 건인지, 어떤 계약의 통지기한이 임박했는지.
2. **계약서 분석·요약** 에서 `파일 선택` 을 눌러 샘플 계약서를 첨부하고 `AI 분석 시작` 을 누릅니다. OCR 판독 → 계약 유형 분류 → 플레이북 기준 대조 → 핵심 내용 요약이 4.8초에 걸쳐 진행됩니다.
3. 결과 화면 좌측에 계약서 원문이 뜹니다. `PDF 원문` 과 `OCR 텍스트` 탭을 오가며 우측의 분류 신뢰도 96%, **플레이북 대비 차이 2건**, 한줄·핵심·조항별 3층 요약과 추출된 대장 필드 9개가 원문의 어느 조항에서 나왔는지 대조합니다.
4. 탐지된 리스크를 **리스크 규칙으로 등록** 하고 `/risk` → `/contracts` 로 이동해 같은 조항을 가진 다른 계약에도 적용되는 것을 보여줍니다. 대시보드로 돌아오면 `조항 노출 현황` 에 방금 등록한 규칙이 `시연 중 등록` 으로 올라와 있습니다.
...
| `src/components/dashboard/*` | 대시보드 블록 — 필터·KPI·조항 노출·만료 타임라인·최근 계약 |
